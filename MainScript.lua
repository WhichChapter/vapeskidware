		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("vape/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/WhichChapter/vapeskidware/"..readfile("vape/commithash.txt").."/CustomModules/"..last) end)
...
task.spawn(function()
	pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/WhichChapter/vapeskidware/main/Libraries/idklol.lua", true))()
	end)
end)
GUI.CreateDivider()
GUI.CreateButton({
	...
	if not shared.VapeIndependent then
		...
		if isfile("vape/CustomModules/"..game.PlaceId..".lua") then
			...
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/WhichChapter/vapeskidware/"..readfile("vape/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
